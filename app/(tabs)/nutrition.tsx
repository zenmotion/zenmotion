...
export default function Nutrition() {
    ...
    const loadMeals = async () => {
        try {
            ...
            const todayMeals = await mealApi.getByUserAndDate(userId, todayStr);
            setRecentMeals(todayMeals.map((meal: any) => ({
                ...meal,
                time: new Date(meal.recorded_at).toLocaleTimeString('pt-BR', {
                    hour: '2-digit',
                    minute: '2-digit',
                }),
            })));
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.macrosCard, { backgroundColor: colors.card.background }]}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.mealTypeScroll}>
                    {mealTypes.map((meal) => (
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                setSelectedMeal({ meal_type: meal.label } as any);
                                setIsModalVisible(true);
                            }}>
            ...
            <View style={styles.recentMeals}>
                ...
                {recentMeals.map((meal) => (
                    <View
                        key={meal.id}
                        style={[styles.mealCard, { backgroundColor: colors.card.background }]}
                    >
